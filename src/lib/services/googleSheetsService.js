...
import { normalizeToCanonicalDate } from "../utils/dateUtils.js";
...
// These header names are the SINGLE SOURCE OF TRUTH for the current "Data" sheet
// (51 columns). Fields are identified by header name only — never by fixed column
// position — so columns can be added/reordered in the sheet without breaking the
// app. This list is only a fallback when the live header row is unavailable; at
// runtime the header map is always built from the LIVE header row (row 0).
...
// widest of these headers so the order / follow-up columns (Number of Followup,
// Next followup Status, Order Status, Order received Date, Remark, Order Number)
// are NEVER overwritten by create/edit — those are managed only by the follow-up
// flow — and only Data rows already owned by the app are rewritten on edit.
  ...
  for (const row of dataRows) {
    ...
    if (!groups.has(quotationNo)) {
      groups.set(quotationNo, {
        ...
        quotationDate: normalizeToCanonicalDate(getCellValue(row, headers, "Quotation Date")),
    ...
    if (detailMap.get(quotationNo).length === 1) {
      ...
      firstRow._quotation = {
        ...
        partyReferenceDate: normalizeToCanonicalDate(getCellValue(row, headers, "Partyref Dt.")),
      ...
      firstRow._followup = {
        ...
        nextFollowupDate: normalizeToCanonicalDate(getCellValue(row, headers, "Next Followup date")),
        nextFollowupStatus: getCellValue(row, headers, "Next followup Status"),
        orderStatus: getCellValue(row, headers, "Order Status"),
        orderReceivedDate: normalizeToCanonicalDate(getCellValue(row, headers, "Order received Date")),
